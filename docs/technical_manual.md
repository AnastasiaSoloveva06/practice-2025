...
```
Этот _обработчи_к ловит нажатия на _inline-кнопки:_ если нажата кнопка FAQ — показывает соответствующий ответ из базы знаний, если нажата "Главное меню" — возвращает пользователя в стартовое меню, а `answer_callback_query` убирает анимацию загрузки у кнопки после нажатия.
